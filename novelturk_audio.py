...
Ortam değişkenleri (hepsi isteğe bağlı)
  ...
  NOVEL_PROXY  http://kullanici:sifre@host:port   (veri merkezi IP'si engelliyse)
...
class CurlFetcher:
    ...
    def __init__(self, proxy=None):
        ...
        self.cffi, self.proxy = cffi, proxy
        try:
...
class Fetcher:
    ...
    def __init__(self, mode="auto", delay=DELAY, retries=RETRIES, proxy=None):
        self.mode, self.delay, self.retries, self.proxy = mode, delay, retries, proxy
    ...
    def _engine(self, name):
        if name not in self._engines:
            self._engines[name] = CurlFetcher(self.proxy) if name == "curl" else PlaywrightFetcher(self.proxy)
    ...
    def _run(self, name, url, referer):
        eng, last = self._engine(name), None
...
def extract_page(html, url, noise=(), min_chars=MIN_CHARS):
    """Ana yöntem trafilatura; sonuç min_chars'ın altındaysa yoğunluk tabanlı yedek denenir."""
    title = page_title(BeautifulSoup(html, "lxml"))
    lines, method = clean_lines(trafilatura_lines(html, url), title, noise), "trafilatura"
    if sum(map(len, lines)) < min_chars:
        alt = clean_lines(density_extract(BeautifulSoup(html, "lxml")), title, noise)
...
def load_page(fx, url, ref, args, pre):
    ...
    page = extract_page(html, url, args.noise, args.min_chars)
    ...
    if fx.can_render and fx.last_engine != "playwright" and (not page.ok or not nxt):
        try:
            ...
            page2, nxt2, better = extract_page(html2, url, args.noise, args.min_chars), find_next(html2, url), False
    ...
    g = guess_next(url) if args.guess else None
...
def scrape_series(url, name, args, fx):
    ...
    try:
        if not chs:
            ...
            _, cur, guessed = load_page(fx, chs[-1]["url"], None, args, pre)
            ref = chs[-1]["url"]
        if chs:
        ...
        while cur:
            ...
            if norm_url(cur) in seen_urls:
                ...
                break
            try:
                page, nxt, nxt_guess = load_page(fx, cur, ref, args, pre)
            except FetchError as e:
                if guessed:
                ...
                break
            digest = hashlib.sha1("\n".join(page.lines).encode()).hexdigest()
...
def _translate_google_one(text, target, source):
    ...
    for chunk in split_text(text, 4500):     # Google'ın tek istekte kabul ettiği karakter sınırının altında
        out.append(GoogleTranslator(source=source or "auto", target=target).translate(chunk) or "")
...
def run_probe(args):
    ...
    print(f"Adres: {url} | fetcher: {args.fetcher} | proxy: {'var' if args.proxy else 'yok'}")
    try:
        for eng in engines:
            t0 = time.time()
            try:
            ...
            print(f"  metin  : {page.chars:,} karakter, {len(page.lines)} paragraf ({page.method})"
                  f"{'' if page.ok else '  <-- YETERSİZ (JS ile yükleniyor olabilir)'}")
            if page.lines:
                print(f"  önizleme: {' / '.join(page.lines[:2])[:220]!r}")
            print(f"  sonraki: {nxt.url + f'  [puan {nxt.score}: {nxt.why}]' if nxt else 'BULUNAMADI'}")
            if not nxt and args.guess:
...
def parse_args(argv=None):
    ...
    p.add_argument("command", nargs="?", default="all",
                   choices=["all", "scrape", "tts", "translate", "epub", "plan", "probe", "voices"])
    # -- kazıma
    ...
    # -- çeviri (translate komutu ve all'da --translate-to ile)
...
def main(argv=None):
    ...
    cmds = {"all": cmd_all, "scrape": cmd_scrape, "tts": cmd_tts, "translate": cmd_translate,
            "epub": cmd_epub, "plan": cmd_plan, "probe": cmd_probe, "voices": cmd_voices}
